...
class SpringSecurityOAuthController {
    ...
    def linkAccount = { OAuthLinkAccountCommand command ->
        ...
        if (request.post) {
            boolean linked = command.validate() && AppUser.withTransaction { status ->
                AppUser user = AppUser.findByUsernameAndPassword(
    ...
    def createAccount = { OAuthCreateAccountCommand command ->
        ...
        if (request.post) {
            if (!springSecurityService.loggedIn) {
                ...
                boolean created = command.validate() && AppUser.withTransaction { status ->
                    AppUser user = new AppUser(username: command.username, password: command.password1, enabled: true)
                    user.addToOAuthIDs(provider: oAuthToken.providerName, accessToken: oAuthToken.socialId, user: user)
                    ...
                    for (roleName in config.oauth.registration.roleNames) {
                        AppUserToRole.create user, Role.findByAuthority(roleName)
                    }
    ...
    protected OAuthToken updateOAuthToken(OAuthToken oAuthToken, AppUser user) {
...
/*
    private def updateUser(AppUser user, OAuthToken oAuthToken) {
        if (!user.validate()) {
            return
        }
...
class OAuthCreateAccountCommand {
    ...
    static constraints = {
        username blank: false, validator: { String username, command ->
            AppUser.withNewSession { session ->
                if (username && AppUser.countByUsername(username)) {
